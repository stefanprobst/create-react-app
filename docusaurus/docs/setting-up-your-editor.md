...
Note that even if you customise the ESLint config, these changes will **only affect the editor integration**. They won’t affect the terminal and in-browser lint output. This is because Create React App intentionally provides a minimal set of rules that find common mistakes.
...
1. This currently only supports configs inside of `package.json`. This is already set for you when creating a new project.
1. You must still extend the base config, as removing it could introduce hard-to-find issues.
